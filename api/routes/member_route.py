...
from fastapi import Depends, HTTPException, APIRouter, Body, Path,Query,status, Request
...
async def register_member(member: Member,user=Depends(verify_token)):
    ...
    # Build full member object with your generated id
    member_data = Member(id=generated_id, **member_dict)

    # Insert into DB
    result = members_collection.insert_one(member_data.model_dump())
...
@router.get("/non-member/{id}", response_model=dict)
async def get_member_by_id(id: str = Path(...)):
    member = non_members_collection.find_one({"id": id})
...
@router.put("/member/update-non-member/{id}", response_model=dict)
async def update_member(id: str = Path(...), update_payload: MemberUpdate = Body(...), user=Depends(verify_token)):
    updates = {k: v for k, v in update_payload.model_dump().items() if v is not None}
    ...
    result = non_members_collection.update_one({"id": id}, {"$set": updates})
...
@router.get("/members/subscription", response_model=list)
async def get_members_subscription(user=Depends(verify_token)):
    members = list(members_collection.find({"amount_subscription": True}))
    for member in members:
        member["_id"] = str(member["_id"])
    return members

@router.get("/members/subscription/count")
async def get_subscription_count(user=Depends(verify_token)):
    count = members_collection.count_documents({"amount_subscription": True})
    return {"paid_members_count": count}
